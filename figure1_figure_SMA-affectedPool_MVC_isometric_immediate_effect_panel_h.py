...
name_fig="Isometric_torque_immediate_effect"
fig2,axes2=plt.subplots(nrows=1, ncols=1, figsize=(4.2/2.54,3.75/2.54))
name_fig2="Isometric_firing_rate_immediate_effect"
fontsize=7
lw=1
lw_axis=0.5
...
for icondition in range(len(NUM_SCS)):
    ...
    print(icondition, np.sum(MN_spikes))

    F=tl.firing_rate_to_force(MN_spikes,max_force, seed)
    Force.append(F)
    ...
    axes2.plot(times,fr,color=colors[icondition])
    mean_force[icondition]=np.mean(F[1500:3500])
    #print("mean_force_num_scs"+str(num_scs),np.mean(F[1500:3500]))
    #fr,times=tl.moving_average(fr, 1,1)

print("%increase",100*(mean_force[1]-mean_force[0])/mean_force[0])
#
# plt.figure()
# plt.plot(times,fr)
# plt.show()
...
hp.yticks(axes,[0,0.3,0.6],fontsize=fontsize)
